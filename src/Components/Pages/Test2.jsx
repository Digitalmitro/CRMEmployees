...
const EmpAttendanceList = () => {
  ...
  const result = {
    ...
    totalFullDays: 0,
    totalLate: 0,
    totalWeekdayHalfDays: 0,
    totalWeekdayFullDays: 0,
    totalWeekends: 0,
  };
  ...
  // calculate full calendar of a month ...
  ...
  const getMissingDates = (datesInMonth, attendanceDates) => {
    const attendanceDateSet = new Set(attendanceDates);
    return datesInMonth.filter((date) => !attendanceDateSet.has(date));
  };

  const getDayOfWeek = (dateStr) => new Date(dateStr).getDay();
  const isWeekend = (dateStr) => [0, 6].includes(getDayOfWeek(dateStr)); // 0 for Sunday, 6 for Saturday

  const categorizeMissingDates = (missingDates) => {
    const result = {
      totalWeekdayAbsent: 0,
      totalWeekends: 0,
    };

    missingDates.forEach((date) => {
      if (isWeekend(date)) {
        result.totalWeekends += 1;
      } else {
        result.totalWeekdayAbsent += 1;
      }
    });
  ...
  useEffect(() => {
    const getEmpAttendanceData = async () => {
      // ... (Existing code to fetch and process attendanceList)
      ...
      const attendanceDates = attendanceList.map((entry) => entry.currentDate);

      // Get missing dates
      const missingDates = getMissingDates(datesInMonth, attendanceDates);

      // Categorize missing dates
      const categorizedMissingDates = categorizeMissingDates(missingDates);

      // Update state or perform any actions needed
      console.log("Categorized Missing Dates:", categorizedMissingDates);
    };
    ...
    const calculateTotals = () => {
      ...
      attendanceList.forEach((entry) => {
        if (entry.status === "Late") {
          totalLate += 1;
        } else if (entry.workStatus === "Half Day") {
          totalHalfDay += 1;
        }
      });
